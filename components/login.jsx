...
const LogIn = (props) => {
  ...
  const handleSubmit = (event) => {
    ...
    axios.post(`${props.mainUrl}/users/sign_in`, {
      user: {
        email: userEmail,
        password: userPass
      }},{
      ...
      .catch(error => {
        setWrongcredentials('Email or Password are wrong!')
      })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col space-y-4 p-8 bg-aux bg-opacity-50 border-b-8 border-aux text-letters-dark rounded-lg">
      <div className="text-center pb-4">
        <h1 className="border-b border-letters-dark rounded-lg text-2xl">Log In</h1>
      </div>
